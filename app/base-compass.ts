...
const APP = {
  name: "Base Compass (Built for Base)",
...
async function readNetworkPulse() {
  const c = client();
...
function mount() {
  ...
  subtitle.textContent =
    "Wallet connection, Base chain validation, and read-only network compass.";
  ...
  btnPulse.textContent = "Network Pulse";
  ...
  btnPulse.onclick = async () => {
    try {
      render(["Reading network pulse…"]);
      const p = await readNetworkPulse();
      render([
        "Network Pulse (read-only)",
        `Network: ${active.label}`,
        `Block: ${p.number}`,
